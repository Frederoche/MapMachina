TileMapMachine.Quadtree = class Quadtree {
    ...
    #testChildCenter(parent, zoomLevel) 
    {
    ...
    #addChilds (node)
    {
        ...
        var nodeOption0 = {
            zoom: zoom,
            key: node.key + "0",
            center: this.#parseKey(node.key + "0"),
            type: 0,
            parent: node
        };
        ...
        var nodeOption1 = {
            zoom: zoom,
            key: node.key + "1",
            center: this.#parseKey(node.key + "1"),
            type: 0,
            parent: node
        };
        ...
        var nodeOption2 = {
            zoom: zoom,
            key: node.key + "2",
            center: this.#parseKey(node.key + "2"),
            type: 0,
            parent: node
        };
        ...
        var nodeOption3 = {
            zoom: zoom,
            key: node.key + "3",
            center: this.#parseKey(node.key + "3"),
            type: 0,
            parent: node
        };
    ...
    getNodeByKey (nodekey, zoom, node) {
    ...
    async traverse (zoom, node) {
        
        node = node || this.RootNode;
        zoom = zoom || TileMapMachine.zoomLevel;

            if (!this.#testChildCenter(node, zoom))
                return;

            if (node.key.length === zoom && (this.Viewport._isInside(node.center, node.key, node.key)) && node.img === null)
            {
                
                 await this.Images.appendImage(node);
            }
            ...
            if (node.key.length < zoom && this.#testChildCenter(node, zoom) && node.child.length === 0) {
                this.#addChilds(node);
                node.type = 1;
            }
